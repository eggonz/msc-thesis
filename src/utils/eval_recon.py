...
def calc_2d_metric(rec_meshfile, gt_meshfile, align=True, n_imgs=1000):
    ...
    try:
        vis.get_render_option().mesh_show_back_face = True  # TODO vis.get_render_option() is None
    ...
    for i in tqdm(range(n_imgs)):
        ...
        ctr = vis.get_view_control()  # TODO vis.get_view_control() is None
...
def eval_recon(rec_mesh,gt_mesh,eval_2d,eval_3d,align):
    ...
    try:
        if eval_3d:
            print('[Eval recon] Evaluating 3D metric')
        ...
        if eval_2d:
            print('[Eval recon] Evaluating 2D metric')
...
def eval_recon_with_cfg(cfg_path,eval_3d=True,eval_2d=False, kf_mesh=True, every_mesh=False):  # FIXME eval2d not working
    print(f'[Eval recon] Evaluating reconstruction from {cfg_path=} ({eval_3d=}, {eval_2d=}, {kf_mesh=}, {every_mesh=})')
    ...
    result = {}
    if kf_mesh:
        print('[Eval recon] Evaluating KF mesh')
        ...
        for k, v in result_kf.items():
            result[f"{k}_kf"]=v
    if every_mesh:
        print('[Eval recon] Evaluating every mesh')
        ...
        for k, v in result_every.items():
            result[f"{k}_every"]=v
    print("[Eval recon] Finished evaluation")
    return result
...
if __name__ == '__main__':
    ...
    result = eval_recon_with_cfg(config_path)
    print("Result:", result)
