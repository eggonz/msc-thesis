...
from src.modules.corr import CorrBlock, AltCorrBlock
...
class FactorGraph:
    ...
    def rm_keyframe(self, ix):
        ...
        with self.video.get_lock():
            ...
            self.video.wq_dirty[ix] = self.video.wq_dirty[ix+1]
    ...
    @torch.cuda.amp.autocast(enabled=True)
    @torch.no_grad()
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False, ba_type="ba"):
        ...
        if t0 is None:
            t0 = max(1, self.ii.min().item()+1)  # t0 starts in 1, t=0 always frozen
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            self.video.ba(target, weight, damping, ii, jj, t0, t1, 
                iters=itrs, lm=1e-4, ep=0.1, motion_only=motion_only,ba_type=ba_type)
    ...
    def update_lowmem(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, steps=8, enable_wq=True):
        ...
        for step in range(steps):
            ...
            if enable_wq:
                ba_type = "ba" if step%2==0 else "wq_ba"
            else:
                ba_type = "ba"

            # dense bundle adjustment            
            self.video.ba(target, weight, damping, self.ii, self.jj, t0, t1, 
                iters=itrs, lm=1e-5, ep=1e-2, motion_only=False, ba_type=ba_type)
