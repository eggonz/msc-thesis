...
from src.common import setup_seed, clone_kf_dict, get_rays_from_uv, update_cam, get_rays
...
class NeuralPointCloud(object):
    def __init__(self, cfg):
        ...
        # self._input_video_idx = []# to save video index of the depth input
        # self._input_j = [] # pixel location of input, depth[j,i]
        # self._input_i = [] # pixel location of input, depth[j,i]
        # self._input_depth = []
    ...
    def add_points(self, video_idxs, pts_gt, mask):
        """add points to NPC in provided indices"""
    ...
    def find_neighbors_faiss(self, pos, step='add', retrain=False, is_pts_grad=False, dynamic_radius=None, use_coarse_pcl=False):
        """
        ...
        if use_coarse_pcl:
            sampled_idx = torch.randperm(nn_num)[:self.nn_num]
            D = D[:,sampled_idx]
            I = I[:,sampled_idx]
    ...
    @torch.no_grad()
    def update_points_pos(self,v_idx,mono_depth,c2w,cfg):        
        ...
        points_depth = mono_depth[points_j, points_i]  
...
def proj_depth_map(c2w, npc, device, cfg, neural_pcl=False, full_res=False):
    H,W,fx,fy,cx,cy = update_cam(cfg, full_res=full_res)
    if neural_pcl:
    ...
    if z.numel() == 0:
        print('WARNING no points found in NPC depth map projection')
        return depth_map
...
def select_points_from_video(video, video_idxs, device):
    """select points from video based on video index"""
    if isinstance(video_idxs, int):
        video_idxs = torch.tensor([video_idxs], dtype=torch.long, device=device)
    intrinsic = video.intrinsics[0].detach() * 8.0
    depth_masks = torch.index_select(video.valid_depth_mask.detach(), dim=0, index=video_idxs)
    disps = torch.index_select(video.disps_up.detach(), dim=0, index=video_idxs)
    poses = torch.index_select(video.poses.detach(), dim=0, index=video_idxs)        
    
    pts_gt = droid_backends.iproj(SE3(poses).inv().data, disps, intrinsic) # [b, h, w 3]
    return pts_gt, depth_masks


def select_points_first_frame(video, depth, device):
    intrinsic = video.intrinsics[0].detach() * 8.0
    depth = depth[None, ...]
    disps = 1.0/depth
    depth_masks = depth > 0
    poses = torch.index_select(video.poses.detach(), dim=0, index=torch.tensor([0], dtype=torch.long, device=device))
    pts_gt = droid_backends.iproj(SE3(poses).inv().data, disps, intrinsic) # [b, h, w 3]
    return pts_gt, depth_masks


def update_points_pos(npc:NeuralPointCloud, video, frame_reader):
    """
    video_idx (list): i.e. [0,1,4]
    """
    with video.get_lock():
    ...
    for i in range(len(video_idx)):
        v_idx = video_idx[i]
        est_depth, est_depth_mask, c2w, _ = video.get_depth_and_pose(v_idx,device=device)
        est_depth[~est_depth_mask] = 0
        c2w[:3, 1:3] *= -1
        if video.cfg["mapping"]["render_depth"] == "droid":
            ...
            _, _, gt_depth, _, _, _, _ = frame_reader[dataset_idx]
            mono_depth = gt_depth.clone().to(device)
            depth_mask = est_depth > 0
            scale_shift = video.get_depth_scale_and_shift(v_idx,mono_depth[depth_mask],
                                                            est_depth[depth_mask], None)
            render_depth = mono_depth*scale_shift[0] + scale_shift[1]
        npc.update_points_pos(v_idx,render_depth.clone(),c2w.clone(),video.cfg)
    pts_gt, mask = select_points_from_video(video, video_idx, device)
    npc.add_points(video_idx, pts_gt, mask)
    
    npc.retrain_updated_points()

def get_droid_render_depth(npc,cfg,c2w, droid_depth, mono_depth, device, idx=None, use_mono_to_complete=True):
    render_depth = droid_depth.clone()
    # droid_valid_mask = droid_depth > 0.0
    # proj_depth = proj_depth_map(c2w, npc, device, cfg)
    # # proj_depth_np = proj_depth.cpu().numpy()
    # # proj_depth_median = cv2.medianBlur(proj_depth_np,5)
    # # proj_depth = torch.from_numpy(proj_depth_median).to(device)
    # proj_valid_mask = (proj_depth > 0.0)
    # proj_mask = (~droid_valid_mask) * proj_valid_mask
    # render_depth[proj_mask] = proj_depth[proj_mask]

    if cfg["mapping"]["save_depth"] and (idx is not None):
        output_path_droid = f"{cfg['data']['output']}/semi_dense_depth/droid/{idx:05d}.npy"
        output_path_proj = f"{cfg['data']['output']}/semi_dense_depth/project/{idx:05d}.npy"
        if not os.path.isfile(output_path_droid):
            droid_depth_np = droid_depth.detach().cpu().float().numpy()
            proj_depth_np = render_depth.detach().cpu().float().numpy()
            np.save(output_path_droid,droid_depth_np)
            np.save(output_path_proj,proj_depth_np)
    if use_mono_to_complete:
        
        if cfg["mapping"]["use_poisson_edit"]:
            raise NotImplementedError("Poisson editing is not implemented")
        else:
            render_invalid_mask = (render_depth == 0)
            render_depth[render_invalid_mask] = mono_depth[render_invalid_mask]
