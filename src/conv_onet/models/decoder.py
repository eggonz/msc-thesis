...
class MLP_geometry(nn.Module):
    ...
    def __init__(self, cfg, c_dim=32,
        ...
        self.N_surface = cfg['rendering']['N_surface_mapper']
    ...
    def get_feature_at_pos(self, npc, p, npc_feats, is_tracker=False, cloud_pos=None,
                           dynamic_r_query=None,use_coarse_pcl=False):
        ...
        if use_coarse_pcl:
            radius_query_bound *= npc.get_coarse_factor()**2
    ...
    def forward(self, p, npc, npc_geo_feats, pts_num=16, is_tracker=False, cloud_pos=None,
                pts_views_d=None, dynamic_r_query=None,use_coarse_pcl=False):
        ...
        c, has_neighbors = self.get_feature_at_pos(
            ...
            use_coarse_pcl=use_coarse_pcl)  # get (N,c_dim), e.g. (N,32)
...
class MLP_color(nn.Module):
    ...
    def forward(self, p, npc, npc_col_feats, is_tracker=False, cloud_pos=None, 
        ...
        c, _ = self.get_feature_at_pos(
            npc, p, npc_col_feats, is_tracker, cloud_pos, dynamic_r_query=dynamic_r_query,use_coarse_pcl=use_coarse_pcl)
        p = p.float().reshape(1, -1, 3)
...
class POINT(nn.Module):
    ...
    def forward(self, p, npc, stage, npc_geo_feats, npc_col_feats, pts_num=16, is_tracker=False, cloud_pos=None,
                pts_views_d=None, dynamic_r_query=None, exposure_feat=None, use_coarse_pcl=False):
        """
        ...
        match stage:
            case 'geometry':
                geo_occ, ray_mask, point_mask, ray_counter = self.geo_decoder(p, npc, npc_geo_feats,
                                                                 ...
                                                                 dynamic_r_query=dynamic_r_query,use_coarse_pcl=use_coarse_pcl)
                raw = torch.zeros(
